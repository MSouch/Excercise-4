...
const ExerciseForm=({exercise,onComplete})=> {
  ...
  useEffect(()=> {
    // Auto-save every 30 seconds while actively filling the form (not after completion/model answer)
  ...
  const handleAutoSave=()=> {
    if (!user || loading || completed || showModelAnswer) return
  ...
  setSaved(true)
  // Hide banner after a short time; will not show when in model answer view
  ...
  const score=calculateScore()
      ...
      setCompleted(true)
  // Immediately show model answer; provide explicit Continue button (no auto-timer)
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {completed && !showModelAnswer ? (
        ...
        <p className="text-gray-600 mb-1">Great work on completing this hands-on exercise.</p>
        <p className="text-gray-600">Review the model answer below.</p>
            </motion.div>
          ) : showModelAnswer ? (
            <motion.div
